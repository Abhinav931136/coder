-- Sample Challenges for InternDesire Platform
-- Run this script after setting up the database

USE interndesire;

-- Insert more sample challenges with comprehensive test cases
...
-- Update user stats for the seed user
UPDATE user_stats 
SET challenges_solved = 3, total_points = 155, current_streak = 3, longest_streak = 5
WHERE user_id = 3;

UPDATE users 
SET points = 155, streak_days = 3
WHERE id = 3;
...
(3, 1, 'python', 'n = int(input())\narr = list(map(int, input().split()))\nprint(sum(arr))', 'accepted', 45, 1024, 25, '[]', NOW()),
(3, 2, 'python', 's = input().strip()\nprint("true" if s == s[::-1] else "false")', 'accepted', 32, 956, 30, '[]', DATE_SUB(NOW(), INTERVAL 1 HOUR)),
(3, 3, 'java', 'import java.util.*;\npublic class Solution {\n    public static void main(String[] args) {\n        Scanner sc = new Scanner(System.in);\n        int n = sc.nextInt();\n        if (n <= 1) {\n            System.out.println(n);\n            return;\n        }\n        int a = 0, b = 1;\n        for (int i = 2; i <= n; i++) {\n            int temp = a + b;\n            a = b;\n            b = temp;\n        }\n        System.out.println(b);\n    }\n}', 'accepted', 78, 2048, 50, '[]', DATE_SUB(NOW(), INTERVAL 2 HOUR));
